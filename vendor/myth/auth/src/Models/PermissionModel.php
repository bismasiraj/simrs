    ...
    public function getPermissionsForUser(int $userId): array
    {
        if (null === $found = cache("{$userId}_permissions")) {
            ...
            $fromGroup = $this->db->table('auth_groups_users')
                ->select('auth_permissions.id, auth_permissions.name, auth_groups_permissions.c,auth_groups_permissions.r,auth_groups_permissions.u,auth_groups_permissions.d')
            ...
            foreach ($combined as $row) {
                // $found[$row->id][strtolower($row->name)]['name'] = strtolower($row->name);
                // $found[$row->id][strtolower($row->name)]['c'] = strtolower($row->c);
                // $found[$row->id][strtolower($row->name)]['r'] = strtolower($row->r);
                // $found[$row->id][strtolower($row->name)]['u'] = strtolower($row->u);
                // $found[$row->id][strtolower($row->name)]['d'] = strtolower($row->d);
                $found[strtolower($row->name)]['name'] = strtolower($row->name);
                $found[strtolower($row->name)]['c'] = strtolower($row->c);
                $found[strtolower($row->name)]['r'] = strtolower($row->r);
                $found[strtolower($row->name)]['u'] = strtolower($row->u);
                $found[strtolower($row->name)]['d'] = strtolower($row->d);
            }
