<script type="text/javascript">
    ...
    const groupeActionPrgInTab = async () => {
    ...
    const groupeActionPrgInAssBid = async () => {
        let visit = <?= json_encode($visit) ?>;
        await getDataPregnancyHistory({
            visit: visit
        })

        $("#arbAddRiwayatHamil").off("click").on("click", function(e) {
            e.preventDefault();
            $("#riwayatHamil-modal").modal("show");
            templatePregnancy({
                visit: visit
    ...
    const templatePregnancy = (props) => {
        const contentShow = [{
            ...
            // },
            {
                ...
                type: "datetime-local",
                name: "partus_date",
                value: props?.data?.partus_date ? props?.data?.partus_date : new Date(),
                id: "prg_partus_date",
                class: ""
            },
            {
        ...
        // initializePrgFlatpickr();
        savePregnancyHistory();
    };

    const initializePrgFlatpickr = () => {
        flatpickr("#prg_flatpartus_date", {
            enableTime: true,
            dateFormat: "d/m/Y H:i", // Display format
            time_24hr: true, // 24-hour time format
        });

        $("#prg_flatpartus_date").prop("readonly", false);

        $("#prg_flatpartus_date").on("change", function() {
    ...
    const renderDataTabelsPregnancyHistory = (props) => {
        ...
        $("#bodydataRiwayatHamil-Asbid").html(resultTables)
        ...
        actionButtonPrgInTabelss(props)
    }


    const actionButtonPrgInTabelss = (props) => {
        ...
        $("#bodydataRiwayatHamil-Asbid").on("click", ".formEditPregnancy", function() {
            const id = $(this).data("id");
            const selectedData = props.data.find(item => item.body_id === id);
            if (selectedData) {
                templatePregnancy({
        ...
        $("#bodydataRiwayatHamil-Asbid").on("click", ".formDeletePregnancy", function() {
            let id = $(this).data('id');
            const selectedData = props.data.find(item => item.body_id === id);
