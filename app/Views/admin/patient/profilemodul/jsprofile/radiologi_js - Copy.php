<script type='text/javascript'>
    $(document).ready(function(e) {
        ...
        initializeSearchTemplateExpertise("template_expertise");
            ...
            success: function(data) {
                ...
                mrJson.forEach((element, key) => {
                    ...
                    $("#radBody").append($("<tr>")
                        ...
                        .append($("<td class='text-center'>").append($("<p>").html(mrJson[key].tarif_name)).append($("<p>").html(mrJson[key].pemeriksaan)).append($("<p>").html(mrJson[key].pemeriksaan_02)).append($("<p>").html(mrJson[key].pemeriksaan_03)))
...
<script>
    ...
    const actionModalExpertise = (bill, identifier, container) => {
        ...
        $('#formExpertise').off('submit').on('submit', e => {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $(`#${identifier}quantity${container}`).val(data.treat_bill.quantity)
                    $(`#${identifier}displayamount_paid${container}`).val(data.treat_bill.amount_paid)
                },
                error: function() {
        ...
        $('#batalExpertise').off().on('click', function() {
            ...
            postData(jsonObj, 'admin/radRequest/cancelExpertise', (res) => {
                if (res.status) {
                    ...
                    $("#modalExpertise").modal("hide")
                    $(`#${identifier}quantity${container}`).val(res.data.quantity)
                    $(`#${identifier}displayamount_paid${container}`).val(res.data.amount_paid)
                } else {
