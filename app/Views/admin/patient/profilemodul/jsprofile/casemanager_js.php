...
(function() {
    ...
    const escapeHtmlCaseManage = (unsafe) => {
        return unsafe
            .replace(/&/g, "&amp;")
            .replace(/"/g, "&quot;")
            .replace(/'/g, "&#039;")
            ...
            if (props) {
                if (parseInt(props.code) === 1) {
                    ...
                    htmlContent = `
                            ...
                            <input type="hidden" class="xxxxx" value="${escapeHtmlCaseManage(props?.data ?? '')}" name="value_info-${props.valueId}" id="hidden-value_info_cm-${props.valueId}">
                            <div id="editor-cm-${props.valueId}" name="value_info-${props.valueId}" style="height: 200px;">${props?.data}</div>
                            `;
                    resolve(htmlContent);
    ...
    const actionViewParamCaseManage = (props) => {
        ...
        filteredData.forEach((e) => {
            ...
            if (!(e.value_score === 9 || e.value_score === 8 || !e.value_score || e.value_desc ===
                    "")) {
                promises.push(getTypeCaseManage({
                    code: e?.value_score,
                    valueId: e?.value_id,
                    valueDesc: e?.value_desc,
                    tb: e?.value_info,
                    parameterId: e?.parameter_id,
                    data: props?.data?.find(f => f.parameter_id === e?.parameter_id)
                        ?.value_info || '',
                }).then((htmlContent) => {
                    let colClass = !e.parameter_id.startsWith('CM_B_') ? 'col-12' :
                        'col-6'; // Adjust column class based on value_score
    ...
    const contentHide = () => {
        return ` <div hidden> <input class="form-control disabled" list="datalistOptions" id="body_id" name="body_id" value='${generateCode()}'>
    ...
    const renderTables1 = (data) => {
        ...
        const tableRows = data.map((item, index) => {
            ...
            <tr>
                ...
                <td>Dokumentasi Case Manager - ${body_id} (${moment(modified_date).format("DD-MM-YYYY : HH:mm")})</td>
                <td width="1%"><button type="button" class="btn btn-sm btn-info btn-show-detail" data-id="${body_id}" data-visit_id="${visit_id}"><i class="fas fa-search-plus"></i> Lihat</button></td>
                <td width="1%"><button type="button" class="btn btn-sm btn-primary btn-show-edit" data-id="${body_id}" data-visit_id="${visit_id}"><i class="fas fa-edit"></i> Ubah</button></td>
                <td width="1%"><button type="button" class="btn btn-sm btn-danger btn-show-delete" data-id="${body_id}" data-visit_id="${visit_id}" data-param_desc="${parameterDesc}"><i class="fas fa-trash-alt"></i> Hapus</button></td>
                <td width="1%"><button type="button" class="btn btn-sm btn-secondary btn-show-print" data-id="${body_id}" data-visit_id="${visit_id}" data-param_desc="${parameterDesc}"><i class="fas fa-print"></i> Print</button></td>
            </tr>
        `;
        }).join('');

        const tableElement = $('#bodydataCM').closest('table');
        if ($.fn.DataTable.isDataTable(tableElement)) {
            tableElement.DataTable().clear().destroy(); // pastikan dihancurkan
        }

        $("#bodydataCM").html(tableRows);
        tableElement.DataTable({
            dom: "tr<'row'<'col-sm-4'p><'col-sm-4 text-center'i><'col-sm-4 text-end'l>>",
            stateSave: false, // ubah sementara jadi false
            bDestroy: true
        });
    ...
    const renderTables = (data) => {
        ...
        const tableElement = $('#table_case_manager').closest('table');

        if (tableElement.length && $.fn.DataTable.isDataTable(tableElement)) {
            tableElement.DataTable().clear().destroy(); // pastikan dibersihkan dulu
        }
            ...
            filteredData.forEach(item => {
                if (!(item.value_score === 9 || item.value_score === 8 || !item.value_score ||
                    ...
                    let getResultArray = resultData.data.filter(dataItem => dataItem.p_type ===
                        "GEN0019" && dataItem.parameter_id === item.parameter_id && dataItem
                    ...
                    if (getResultArray.length > 0) {
                        if (item.value_score === 7) {
                            promises.push(getTypeCaseManage({
                                ...
                                data: resultData?.data?.find(f => f.parameter_id ===
                                        item
                                        ?.parameter_id)
                                    ?.value_info || '',
                            }).then((htmlContent) => {
    ...
    const modalViewEdit = (data) => {
        ...
        let result = resultData.data[0];
        const uniqueParams = [
            ...new Map(resultData.data.map(item => [item.parameter_id, {
