    ...
    const dateLab = () => {
        ...
        $("#startDateLab").on('change', function() {
            ...
            if (moment(endDate).isBefore(moment(startDate))) {
                $("#endDateLab").val(startDate);
            }
            $("#endDateLab").attr('min', startDate);
        });
        ...
        $("#btn-search-lab").off().on("click", function() {
            const start = moment($("#startDateLab").val()).format("YYYY-MM-DD") + " 00:00:01";
            const end = moment($("#endDateLab").val()).format("YYYY-MM-DD") + " 23:59:59";
            getBillPoli(nomor, ke, mulai, akhir, lunas, 'P013', rj, status, nota, trans, start, end);
            $("#notaNoLab").val("%")
        });
    };
...
<script type='text/javascript'>
    function addBillLab(container) {
        ...
        $("#labChargesBody").append($("<tr id=\"" + key + "\">")
            ...
            .append($("<td>").attr("id", "alabdisplaytreatment" + key).html(tarifData.tarif_name).append($("<p>").html(
                '<?= $visit['fullname']; ?>')))
