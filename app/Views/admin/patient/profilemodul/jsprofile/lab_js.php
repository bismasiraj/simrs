            ...
            if (!(tarifIds.includes(tarifData.tarif_id) || tarifIdsInResult.includes(tarifData.tarif_id))) {
                ...
                if ('<?= $visit['class_id']; ?>' != '<?= $visit['class_id_plafond']; ?>') {
                    ...
                    if (tarifKelas > 0 && '<?= $visit['payor_id']; ?>' != 0 &&
                        ...
                        99) {
                        $("#alab" + key).append('<input name="amount_plafond[]" id="alabamount_plafond' + key +
                            '" type="hidden" value="' + tarifKelas + '" class="form-control" />')
                        $("#alab" + key).append('<input name="amount_paid_plafond[]" id="alabamount_paid_plafond' +
                            key +
                            '" type="hidden" value="' + tarifKelas + '" class="form-control" />')
                        $("#alab" + key).append('<input name="class_id_plafond[]" id="alabclass_id_plafond' + key +
                            '" type="hidden" value="<?= $visit['class_id_plafond']; ?>" class="form-control" />'
                        )
                        $("#alab" + key).append('<input name="tarif_id_plafond[]" id="alabtarif_id_plafond' + key +
...
<script>
            ...
            if (groupedData.hasOwnProperty(kelPemeriksaan)) {
                ...
                for (let tarifName in groupedData[kelPemeriksaan]) {
                    if (groupedData[kelPemeriksaan].hasOwnProperty(tarifName)) {
                        const isGambaranDarahTepi = tarifName.toLowerCase().includes("gambaran darah tepi");

                        result += `<tr>
                                                <td colspan="6" style="padding-left: 20px;"><strong>${tarifName}</strong></td>
                                            </tr>`;
                        if (isGambaranDarahTepi) {
                            ...
                            groupedData[kelPemeriksaan][tarifName].forEach(e => {
                                result += `<tr>
                                                   ...
                                                   <td>
                                                        ${!e.nilai_rujukan || e.nilai_rujukan === '&nbsp;' 
                                                            ? "-" 
                                                            : e.nilai_rujukan.replace(/\n/g, "<br>")}
                                                        </td>
                                                    <td>${!e.catatan? "-": e.catatan === "-" ? !e.rekomendasi ? "-" : e.rekomendasi : e.catatan }</td>
    ...
    const populateLabHasilTable = () => {
        ...
        hasilLIS.map((item, index) => {
            ...
            <td class="text-center align-middle">
                <?php if (user()->checkPermission("lab", 'c') && user()->checkRoles(['dokterlab', 'superuser', 'adminlab'])) { ?>
                    <input type="checkbox" class="validator-checkbox me-2"
                        value="${item?.kode_kunjungan}" 
                        data-lis="${item?.nolab_lis}"
                        ${item?.valid_user ? "disabled" : ""}>
                <?php } ?>
                ${index + 1}
            </td>
            <td class="text-center align-middle">
                ${item?.reg_date ? moment(item?.reg_date).format("DD/MM/YYYY HH:mm") : item?.reg_date ?? "-"}
                <input name="kodekunjungan" id="addKj${item?.kode_kunjungan}" value="${item?.kode_kunjungan}" type="hidden"> 
            </td>
            <td class="text-center align-middle">
                ${item?.tarif_names ? item?.tarif_names.replace(/&nbsp;/g, ' ') : ''}
            </td>
            <td class="text-center align-middle">
        ...
        $("button[name='signLabValidAction']").off().on("click", function() {
            ...
            const checkedBoxes = $(".validator-checkbox:checked");
            const validUserId = $("#user_id").val();

            if (checkedBoxes.length > 0) {
                let batchData = [];
                checkedBoxes.each(function() {
                    const kodeKunjungan = $(this).val();
                    const nolis = $(this).data("lis");
                    batchData.push({
                        kodeKunjungan,
                        nolis,
                ...
                actionModalValidaton(batchData);
            } else {
                const kodeKunjungan = $(this).data("id");
                const nolis = $(this).data('lis');
    ...
    const actionModalValidaton = (props) => {
        if (Array.isArray(props)) {
            const promises = props.map(d => {
                let data = {
                    nolis: d.nolis,
                    kodeKunjungan: d.kodeKunjungan,
                    valid: d.valid,
                };

                return new Promise((resolve, reject) => {
                    postData(data, 'admin/labRequest/updateValidation', (res) => {
                        if (res?.respon === true) {
                            resolve(true);
                        } else {
                            reject(false);
            ...
            Promise.all(promises)
                ...
                .catch(() => {
                    errorSwal("Ada yang gagal divalidasi");
                });
