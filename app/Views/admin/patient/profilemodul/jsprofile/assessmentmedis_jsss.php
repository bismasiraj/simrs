...
<script type="text/javascript">
    ...
    const initialAddArm = async () => {
        ...
        if (assessmentMedisType == 1) {
            ...
            renderViewAssessmentMedis(accMedisName, pasienDiagnosaId, 1)
        }
    ...
    const renderViewAssessmentMedis = async (accMedisName, pasienDiagnosaId, flag = 1) => {
        var titlerj = '';
    ...
    const copyLastArm = async () => {
        ...
        if (pasienDiagnosaAll?.length > 0) {
            pasienDiagnosa = getLastObject(sortAscending(pasienDiagnosaAll, 'date_of_diagnosa'));
            if (pasienDiagnosa) {
                $("#armdescription").val(pasienDiagnosa?.description)
                $("#armanamnase").val(pasienDiagnosa?.anamnase)
                $("#armalloanamnase").val(pasienDiagnosa?.alloanamnase)
                // $("#armmedical_problem").val(pasienDiagnosa?.medical_problem)
                $("#armhurt").val(pasienDiagnosa?.hurt)
                $("#arminstruction").val(pasienDiagnosa?.instruction)
                $("#armdiagnosa_desc").val(pasienDiagnosa?.diagnosa_desc)
            }
    ...
    const fillDataArm = async (index) => {
        ...
        console.log(pasienDiagnosa.doctor)
        ...
        if (assessmentMedisType == 1) {
            ...
            renderViewAssessmentMedis(accMedisName, pasienDiagnosa.pasien_diagnosa_id, 0)
        }
                        ...
                        .append(`<tr class="${rowColor}">
                                    ...
                                    <td rowspan="6">
                                        <div class="btn-group-vertical" role="group" aria-label="Vertical button group">
                                            ...
                                            <button type="button" class="btn btn-light" onclick="openPopUpTab('<?= base_url() ?>admin/rm/medis/${linkcetak}/<?= base64_encode(json_encode($visit)); ?>/${value?.pasien_diagnosa_id}/${titlerj}')">Cetak</button>
                                        </div>
                                    </td>
                                </tr>
                            `)
...
<script>
    ...
    function appendVitalSignMedis(accordionId) {
        $("#" + accordionId).append(
            ...
            '<div class="accordion-body text-muted">' +
            '<h5>Vital Sign <a id="copyPeriksaFisikBtn" href="#" onclick="refreshPeriksaFisik()">(refresh)</a></h5>' +
