...
<script type="text/javascript">
    ...
    const btnSaveAction = () => {
        $("#btn-save-inf-modal").off().on("click", function(e) {
            ...
            postData(jsonObj, 'admin/informendconsent/insertData', (res) => {
                if (res.respon === true) {
    ...
    const getDataTable = (props) => {
        $("#infConsentTap").off().on("click", function(e) {
            e.preventDefault();
            let visit_id = '<?php echo $visit['visit_id']; ?>';
            getDataTables({
                visit_id: visit_id
    ...
    const getDataTables = (props) => {
        ...
        }, 'admin/InformendConsent/getData', (res) => {
            if (res.length >= 1) {
                renderTables(res);
            } else {
    ...
    const getDataRead = () => {
        $('.btn-show-detail').on('click', function(e) {
            ...
            }, 'admin/InformendConsent/getDetail', (res) => {
                tinymce.remove();
                modalViewDetail({
                    data: res
                });
    ...
    const btnUpdateData = () => {
        $('#btn-edit-inf-modal').off().on("click", (e) => {
            ...
            putData(jsonObj, 'admin/InformendConsent/updateData', (res) => {
                if (res.respon === true) {
    ...
    const renderTables = (data) => {
        ...
        let dataResult = [];
        data.forEach(item => {
            const key = `${item.body_id}_${item.parameter_id}_${item.visit_id}_`;
            if (!groupedData[key]) {
                groupedData[key] = [];
            }
        ...
        const tableRows = Object.keys(groupedData).map((key, index) => {
            const [bodyId, parameterId, visitId] = key.split("_");
            const parameterDesc = fill.find(param => param.parameter_id === parameterId).parameter_desc;
            return `
                <tr>
                    <td>${index +1}</td>
                    <td>${parameterDesc}</td>
    ...
    const actionCetak = () => {
        $(".btn-show-print").on('click', function(e) {
            postData({
                ...
                let visitencode = <?= json_encode($visit); ?>;
                let hasil = {
                    data: res
                };
                hasilData(hasil);
            ...
            const hasilData = (data) => {
                localStorage.setItem('print', JSON.stringify(data));
                let url = "<?= base_url() . 'admin/InformendConsent/cetakData/' . base64_encode(json_encode($visit));  ?>";
