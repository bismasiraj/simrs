...
<script type='text/javascript'>
    ...
    const declareSearchTarifFisio = () => {
        initializeSearchTarif("searchTariffisio", 'P015');
        $("#searchTariffisio").on('select2:select', function(e) {
            $("#searchTariffisioBtn").click();
            $('html,body').animate({
                    scrollTop: $("#searchTariffisio").offset().top - 50
                },
                'slow', 'swing');
            $("#searchTariffisio").click()
            $("#searchTariffisio").select2('open')
        });
    }
    ...
    function addBillfisio(container) {
        ...
        if (nota_no == '%') {
            $("#notaNofisio").find(`option[value='${sesi}']`).remove()
            nota_no = sesi
        ...
        var key = 'fisio' + i
        $("#fisioChargesBody").append($("<tr id=\"afisio" + key + "\">")
            .append($("<td>").html(String(i) + "."))
            .append($("<td>").attr("id", "afisiodisplaytreatment" + key).html(tarifData.tarif_name).append($("<p>")
                .html('<?= @$visit['fullname']; ?>')))
            .append($("<td>").html('<select id="afisioemployee_id' + key +
                '" class="form-select" name="employee_id[]" readonly>' +
                chargesDropdownDoctor() +
                `</select>`))
            .append($("<td>").attr("id", "afisiodisplaytreat_date" + key).html(moment().format("DD/MM/YYYY HH:mm"))
                .append($("<p>").html('<?= $visit['name_of_clinic']; ?>')))
            // .append($("<td>").attr("id", "iscetak" + key).html(billJson[key].iscetak))
            .append($("<td>").attr("id", "afisiodisplaysell_price" + key).html(formatCurrency(parseFloat(tarifData
                .amount))).append($("<p>").html("")))
            .append($("<td>")
                .append('<input type="text" name="quantity[]" id="afisioquantity' + key +
                    '" placeholder="" value="0" class="form-control" >')
                .append($("<p>").html('<?= $visit['name_of_status_pasien']; ?>'))
            )
            .append($("<td>").attr("id", "afisiodisplayamount_paid" + key).html(formatCurrency(parseFloat(tarifData
                .amount))))
            .append($("<td>").append('<button id="afisiosimpanBillBtn' + key +
                '" type="button" onclick="simpanBillCharge(\'' + key +
                '\', \'afisio\')" class="btn btn-info waves-effect waves-light simpanbill" data-row-id="1" autocomplete="off">Simpan</button><div id="afisioeditDeleteCharge' +
                key +
                '" class="btn-group-vertical" role="group" aria-fisioel="Vertical button group" style="display: none"><div class="btn-group-vertical" role="group" aria-fisioel="Vertical button group"><button id="editBillBtn' +
                key + '" type="button" onclick="editBillCharge(\'afisio\', \'' + key +
                '\')"class="btn btn-success waves-effect waves-light" data-row-id="1" autocomplete="off">Edit</button><button id="delBillBtn' +
                key + '" type="button" onclick="delBill(\'afisio\', \'' + key +
                '\')" class="btn btn-danger" data-row-id="1" autocomplete="off">Hapus</button></div>'))
        )


        $("#fisioChargesBody")
            .append('<input type="hidden" name="quantity[]" id="afisioquantity' + key +
                '" placeholder="" value="0" class="form-control" >')
            .append('<input name="treatment[]" id="afisiotreatment' + key + '" type="hidden" value="' + tarifData
                .tarif_name + '" class="form-control" />')
            .append('<input name="treat_date[]" id="afisiotreat_date' + key + '" type="hidden" value="' + get_date() +
                '" class="form-control" />')
            .append('<input name="sell_price[]" id="afisiosell_price' + key + '" type="hidden" value="' + tarifData.amount +
                '" class="form-control" />')
            .append('<input name="amount_paid[]" id="afisioamount_paid' + key + '" type="hidden" value="' + tarifData
                .amount + '" class="form-control" />')
            .append('<input name="discount[]" id="afisiodiscount' + key + '" type="hidden" value="' + 0 +
                '" class="form-control" />')
            .append('<input name="subsidisat[]" id="afisiosubsidisat' + key + '" type="hidden" value="' + 0 +
                '" class="form-control" />')
            .append('<input name="subsidi[]" id="afisiosubsidi' + key + '" type="hidden" value="' + 0 +
                '" class="form-control" />')

            .append(
                `<input name="bill_id[]" id="afisiobill_id${key}" type="hidden" value="${billJson?.bill_id ?? codeData}"  class="form-control" />`
            )
            .append('<input name="trans_id[]" id="afisiotrans_id' + key +
                '" type="hidden" value="<?= $visit['trans_id']; ?>" class="form-control" />')
            .append('<input name="no_registration[]" id="afisiono_registration' + key +
                '" type="hidden" value="<?= $visit['no_registration']; ?>" class="form-control" />')
            .append('<input name="theorder[]" id="afisiotheorder' + key + '" type="hidden" value="' + (billJson.length +
                1) + '" class="form-control" />')
            .append('<input name="visit_id[]" id="afisiovisit_id' + key +
                '" type="hidden" value="<?= $visit['visit_id']; ?>" class="form-control" />')
            .append('<input name="org_unit_code[]" id="afisioorg_unit_code' + key +
                '" type="hidden" value="<?= $visit['org_unit_code']; ?>" class="form-control" />')
            .append('<input name="class_id[]" id="afisioclass_id' + key +
                '" type="hidden" value="<?= $visit['class_id']; ?>" class="form-control" />')
            .append('<input name="class_id_plafond[]" id="afisioclass_id_plafond' + key +
                '" type="hidden" value="<?= $visit['class_id_plafond']; ?>" class="form-control" />')
            .append('<input name="payor_id[]" id="afisiopayor_id' + key +
                '" type="hidden" value="<?= $visit['payor_id']; ?>" class="form-control" />')
            .append('<input name="karyawan[]" id="afisiokaryawan' + key +
                '" type="hidden" value="<?= $visit['karyawan']; ?>" class="form-control" />')
            .append('<input name="theid[]" id="afisiotheid' + key +
                '" type="hidden" value="<?= $visit['pasien_id']; ?>" class="form-control" />')
            .append('<input name="thename[]" id="afisiothename' + key +
                '" type="hidden" value="<?= $visit['diantar_oleh']; ?>" class="form-control" />')
            .append('<input name="theaddress[]" id="afisiotheaddress' + key +
                '" type="hidden" value="<?= $visit['visitor_address']; ?>" class="form-control" />')
            .append('<input name="status_pasien_id[]" id="afisiostatus_pasien_id' + key +
                '" type="hidden" value="<?= $visit['status_pasien_id']; ?>" class="form-control" />')
            .append('<input name="isrj[]" id="afisioisrj' + key +
                '" type="hidden" value="<?= $visit['isrj']; ?>" class="form-control" />')
            .append('<input name="gender[]" id="afisiogender' + key +
                '" type="hidden" value="<?= $visit['gender']; ?>" class="form-control" />')
            .append('<input name="ageyear[]" id="afisioageyear' + key +
                '" type="hidden" value="<?= $visit['ageyear']; ?>" class="form-control" />')
            .append('<input name="agemonth[]" id="afisioagemonth' + key +
                '" type="hidden" value="<?= $visit['agemonth']; ?>" class="form-control" />')
            .append('<input name="ageday[]" id="afisioageday' + key +
                '" type="hidden" value="<?= $visit['ageday']; ?>" class="form-control" />')
            .append('<input name="kal_id[]" id="afisiokal_id' + key +
                '" type="hidden" value="<?= $visit['kal_id']; ?>" class="form-control" />')
            .append('<input name="karyawan[]" id="afisiokaryawan' + key +
                '" type="hidden" value="<?= $visit['karyawan']; ?>" class="form-control" />')
            .append('<input name="class_room_id[]" id="afisioclass_room_id' + key +
                '" type="hidden" value="<?= $visit['class_room_id']; ?>" class="form-control" />')
            .append('<input name="bed_id[]" id="afisiobed_id' + key +
                '" type="hidden" value="<?= $visit['bed_id']; ?>" class="form-control" />')
            .append('<input name="clinic_id[]" id="afisioclinic_id' + key +
                '" type="hidden" value="P015" class="form-control" />')
            .append('<input name="clinic_id_from[]" id="afisioclinic_id_from' + key +
                '" type="hidden" value="<?= $visit['clinic_id_from']; ?>" class="form-control" />')
            .append('<input name="exit_date[]" id="afisioexit_date' + key + '" type="hidden" value="' + get_date() +
                '" class="form-control" />')
            .append('<input name="cashier[]" id="afisiocashier' + key +
                '" type="hidden" value="<?= user_id(); ?>" class="form-control" />')
            .append('<input name="modified_from[]" id="afisiomodified_from' + key +
            ...
            .append('<input name="measure_id[]" id="afisiomeasure_id' + key +
                '" type="hidden" value="" class="form-control" />')
            .append('<input name="tarif_id[]" id="afisiotarif_id' + key + '" type="hidden" value="' + tarifData.tarif_id +
                '" class="form-control" />')

        $("#afisioemployee_id" + key).val(visit.employee_id)
        ...
        if ('<?= $visit['isrj']; ?>' == '0') {
            ...
            ?>
                $("#fisioChargesBody")
                    .append('<input name="employee_id_from[]" id="afisioemployee_id_from' + key +
                        '" type="hidden" value="<?= $visit['employee_id_from']; ?>" class="form-control" />')
                    .append('<input name="doctor_from[]" id="afisiodoctor_from' + key +
        ...
        $("#afisioemployee_id_from" + key).val('<?= user()->employee_id; ?>')
        $("#afisiodoctor_from" + key).val('<?= user()->getFullname(); ?>')
        $("#afisioemployee_id" + key).val('<?= user()->employee_id; ?>')
        $("#afisiodoctor" + key).val('<?= user()->getFullname(); ?>')

        $("#fisioChargesBody")
            .append('<input name="employee_id[]" id="afisioemployee_id' + key +
                '" type="hidden" value="<?= $visit['employee_id']; ?>" class="form-control" />')
            .append('<input name="doctor[]" id="afisiodoctor' + key +
                '" type="hidden" value="<?= @$visit['fullname']; ?>" class="form-control" />')
            .append('<input name="amount[]" id="afisioamount' + key + '" type="hidden" value="' + tarifData.amount +
                '" class="form-control" />')
            .append('<input name="nota_no[]" id="afisionota_no' + key + '" type="hidden" value="' + nota_no +
                '" class="form-control" />')
            .append('<input name="profesi[]" id="afisioprofesi' + key + '" type="hidden" value="" class="form-control" />')
            .append('<input name="tagihan[]" id="afisiotagihan' + key + '" type="hidden" value="' + tarifData.amount * $(
                "#afisioquantity" + key).val() + '" class="form-control" />')
            .append('<input name="treatment_plafond[]" id="afisiotreatment_plafond' + key + '" type="hidden" value="' +
                tarifData.amount + '" class="form-control" />')
            .append('<input name="tarif_type[]" id="afisiotarif_type' + key + '" type="hidden" value="' + tarifData
        ...
        if ('<?= $visit['class_id']; ?>' != '<?= $visit['class_id_plafond']; ?>') {
            ...
            if (tarifKelas > 0 && '<?= $visit['payor_id']; ?>' != 0 && '<?= $visit['class_id_plafond']; ?>' != 99) {
                $("#fisioChargesBody").append('<input name="amount_plafond[]" id="afisioamount_plafond' + key +
                    '" type="hidden" value="' + tarifKelas + '" class="form-control" />')
                $("#fisioChargesBody").append('<input name="amount_paid_plafond[]" id="afisioamount_paid_plafond' + key +
                    '" type="hidden" value="' + tarifKelas + '" class="form-control" />')
                $("#fisioChargesBody").append('<input name="class_id_plafond[]" id="afisioclass_id_plafond' + key +
                    '" type="hidden" value="<?= $visit['class_id_plafond']; ?>" class="form-control" />')
                $("#fisioChargesBody").append('<input name="tarif_id_plafond[]" id="afisiotarif_id_plafond' + key +
        ...
        $('#afisioquantity' + key).on("input", function() {
            ...
            $("#afisiodisplayamount_paid_plafond" + key).html(formatCurrency($("#afisioamount_plafond" + key)
                .val() * dInput))
        })
    }
</script>
