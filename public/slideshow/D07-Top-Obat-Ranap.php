<?php
  ...
  $qObatRanap = 'execute dbo.web_D07_Top_Obat_Ranap';
  $rObatRanap = querryResult($dbconnect,$qObatRanap);
 ...
 ?>
          ...
          "titles": [
            {
              "id": "Title-1",
              "size": 15,
              "text": "Top 10 Obat Rawat Inap"
            }
          ],
          "dataProvider": [
            <?php foreach ($rObatRanap as $key => $value): ?>
              {
                <?php echo "\"category\": \"".$rObatRanap[$key]['description']."\"," ?>
                <?php echo "\"column-1\": ".$rObatRanap[$key]['jml']."," ?>
                "color": "#FB6542"
              },
            <?php endforeach; ?>
          ]
     ...
     <!-- </div> -->
     <?php
     $qSlide = 'select timeslide from WEBSITE_MENU where MENU_ID=7';
